...
# 현재 파일(`deoksugung.py`)이 위치한 디렉토리 찾기
base_dir = os.path.dirname(os.path.abspath(__file__))

# dataset_filtered_csv 폴더에 있는 파일의 상대 경로 지정
file_path = os.path.join(base_dir, "..", "dataset_filtered_csv", "덕수궁_filtered2.csv")
...
# Feature와 Label 분리
X = df.drop(columns=['덕수궁']).iloc[1:315]  # Feature
y = df['덕수궁'].iloc[1:315]  # Label
...
# 현재 실행 중인 파일(`deoksugung.py`)이 위치한 디렉토리 찾기
base_dir = os.path.dirname(os.path.abspath(__file__))
...
result = {
    "덕수궁": {
        ...
        "congestion_level": int((y_live_pred.tolist()[0] / 20401) * 100)
    }
}
