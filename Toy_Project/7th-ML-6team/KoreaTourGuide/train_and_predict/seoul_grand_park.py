...
# 현재 파일(`seoulpark.py`)이 위치한 디렉토리 찾기
base_dir = os.path.dirname(os.path.abspath(__file__))

# dataset_filtered_csv 폴더에 있는 파일의 상대 경로 지정
file_path = os.path.join(base_dir, "..", "dataset_filtered_csv", "서울대공원_filtered2.csv")
...
# Feature와 Label 분리
X = df.drop(columns=['서울대공원']).iloc[1:315]  # Feature
y = df['서울대공원'].iloc[1:315]  # Label
...
# 현재 실행 중인 파일(`seoul_grand_park.py`)이 위치한 디렉토리 찾기
base_dir = os.path.dirname(os.path.abspath(__file__))
...
result = {
    "서울대공원": {
        ...
        "congestion_level": int((y_live_pred.tolist()[0] / 1984065) * 100)
    }
}
